cordova.define("kapsel-plugin-xhook.Xhook", function(require, exports, module) {
...
fakeEvent = function(type) {
  ...
  if (document.createEventObject != null) {
    ...
    try {
      // Kapsel modification for Windows platform.
  ...
  switch (typeof h) {
    case "object":
      ...
      for (k in h) {
        v = h[k];
        name = k.toLowerCase();
        // Kapsel modification: a single space is preferred over other
        // forms of whitespace according to the HTTP spec.
        headers.push("" + k + ": " + v);
      }
      // Kapsel modification: Must use \r\n to comform to spec.
      return headers.join('\r\n');
    case "string":
      headers = h.split('\n');
...
XHookHttpRequest = window[XMLHTTP] = function() {
  ...
  emitReadyState = function(n) {
    while (n > currentState && currentState < 4) {
      ...
      if (currentState === 4) {
          //This change is required because SAP UI5 starting from 1.40 throws an exception when the setTimeout function is called here
          if (request.async) {
              //For asynchronous calls the setTimeout doesn't cause any issue and this is the original version
              setTimeout(emitFinal, 0);
          } else {
              //The SAP UI5 call is synchronous so calling it without setTimeout doesn't cause any issue
              emitFinal();
          }
      }
    }
  ...
  facade.open = function(method, url, async, user, pass) {
    ...
    request.url = url;
    // Kapsel modification: Allow synchronous requests
    request.async = async;
    request.user = user;
    request.pass = pass;
    setReadyState(1);
  ...
  facade.send = function(body) {
    ...
    send = function() {
      ...
      transiting = true;
      // Kapsel modification
      // BCP 504189 / 2019 / xhook is not calling the function for XMLHttpRequests correctly / SAP Delivered Support
      //xhr.open(request.method, request.url, request.async, request.user, request.pass);
      xhr.open(
         request.method,
         request.url,
         typeof request.async === 'undefined' ? true : request.async,
         typeof request.user === 'undefined' ? '' : request.user,
         typeof request.pass === 'undefined' ? '' : request.pass
	  );
...
xhook.disable(); // Disable xhook by default.
