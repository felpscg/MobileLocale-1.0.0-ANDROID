cordova.define("kapsel-plugin-logon.Logon", function(require, exports, module) {
    ...
    var getAppConfig = function(context, errorCallback) {
        ...
        if (context.operation.currentConfigState == "run") {
            ...
            if (configType == "saved" || configType == "appConfig.js") {
                ...
                //if this happens after restarting the webview, then no need to ask user to input the fiori url again, just submit the screen with saved fiori url
                if (context.operation.fioriConfiguration){
                    var screenContext = {};
                    screenContext.fioriConfiguration = context.operation.fioriConfiguration;
                    if (appDelegate && appDelegate.onSubmitScreen) {
                        appDelegate.onSubmitScreen(context, screenContext, onGetConfigData, appDelegate.onRegistrationError);
                    }
                    return;
                }
    ...
    var init = function(successCallback, errorCallback, applicationId, context, customView, credentialProviderID) {
        ...
        document.addEventListener("pause", _pauseListener,false);
        ...
        var callChangePasscode = function(context) {
            ...
            sap.logon.Core.changePasscode(
                function(currentContext, state){
                    ...
                    if (device.platform.toLowerCase() == "ios" || device.platform.toLowerCase() == "android"){
                        sap.logon.Core.setFingerprintEnabled(function(){onCoreResult(currentContext, state)}, function(e){
                            utils.log("setFingerprintEnabled error callback (RegistrationFlow>callChangePasscode): " + JSON.stringify(e));
                            // User cancelled the fingerprint authentication or some error occurred.
                            // Just continue without fingerprint.
                            onCoreResult(currentContext, state);
                        }, currentContext);
                    }
                    else{
                ...
                if (context.passcodeEnabled) {
                    ...
                    logonCore.persistRegistration(
                        function(currentContext, state){
                            ...
                            if (device.platform.toLowerCase() == "ios" || device.platform.toLowerCase() == "android"){
                                sap.logon.Core.setFingerprintEnabled(function(){onCoreResult(currentContext, state)}, function(e){
                                    utils.log("setFingerprintEnabled error callback (RegistrationFlow>onCreatePasscodeSubmit): " + JSON.stringify(e));
                                    // User cancelled the fingerprint authentication or some error occurred.
                                    // Just continue without fingerprint.
                                    onCoreResult(currentContext, state);
                                }, currentContext);
        ...
        this.stateTransitions = [{
            ...
            {   // This rule is for when the Afaria configuration contains "vaultpolicy=alwaysoff;"
                ...
                condition: {
                    ...
                    method: function(state, context) {
                        return context&&context.policyContext&&context.policyContext.alwaysOff;
                    }
                },
                ...
                if (context.passcodeEnabled) {
                    ...
                    logonCore.createSecureStore(
                        function(currentContext, state){
                            ...
                            if (device.platform.toLowerCase() == "ios" || device.platform.toLowerCase() == "android"){
                                sap.logon.Core.setFingerprintEnabled(function(){onCoreResult(currentContext, state)}, function(e){
                                    utils.log("setFingerprintEnabled error callback (noRegistration>onCreateSecureStoreSubmit): " + JSON.stringify(e));
                                    // User cancelled the fingerprint authentication or some error occurred.
                                    // Just continue without fingerprint.
                                    onCoreResult(currentContext, state);
                                }, currentContext);
        ...
        var callChangePasscode = function(context) {
            ...
            sap.logon.Core.changePasscode(
                function(currentContext, state){
                    ...
                    if (device.platform.toLowerCase() == "ios" || device.platform.toLowerCase() == "android"){
                        sap.logon.Core.setFingerprintEnabled(function(){onCoreResult(currentContext, state)}, function(e){
                            utils.log("setFingerprintEnabled error callback (noRegistration>callChangePasscode): " + JSON.stringify(e));
                            // User cancelled the fingerprint authentication or some error occurred.
                            // Just continue without fingerprint.
                            onCoreResult(currentContext, state);
                        }, currentContext);
        ...
        var callChangePasscode = function(context) {
            ...
            logonCore.changePasscode(
                function(currentContext, state){
                    ...
                    currentContext.passcode = context.passcode;
                    if (device.platform.toLowerCase() == "ios" || device.platform.toLowerCase() == "android"){
                        sap.logon.Core.setFingerprintEnabled(function(){onFlowSuccess(currentContext, state)}, function(e){
                            utils.log("setFingerprintEnabled error callback (ManagePasscodeFlow>callChangePasscode): " + JSON.stringify(e));
                            // User cancelled the fingerprint authentication or some error occurred.
                            // Just continue without fingerprint.
   ...
   /**
    ...
    var isSapLogonFired = function(){
        return _hasLogonSuccessEventFired || _isUnderForegroundState;
    }
    ...
    module.exports = {
        ...
        * }
        ...
        /**
         * return true if 'onSapResumeSuccess' or 'onSapLogonSuccess' is fired and the pause or unregister is never raised
         */
        isSapLogonFired: isSapLogonFired
    };
