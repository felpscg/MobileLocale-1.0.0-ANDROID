...
AppPreferences = function() {
  ...
  /**
   * Delete a key-value pair for a preference.
   * iOS platform only
   * @memberof sap.AppPreferences
   * @function deletePreferenceValue
   * @param {String} key   a unique identifier for a preference value.
   * @param {Function} [successCallback]   Callback function called when a key-value pair for a preference has been successfully deleted.
   * @param {Function} [errorCallback]   Callback function called when an error occurs while deleting a key-value pair for a preference.
   * @example
   * sap.AppPreferences.deletePreferenceValue(function() {
   *   "name_preference",
   *    function(successMsg) {alert("Succeess: " + successMsg);},
   *    function(errMsg) {alert("Error: " + errMsg);} );
   */
   this.deletePreferenceValue = function (key, successCallback, errorCallback) {
       if (cordova.require("cordova/platform").id == "ios"){
            exec(successCallback, errorCallback, "AppPreferences", "deletePreferenceValue", [key]);
       }
       else{
            if (typeof errorCallback == "function"){
                errorCallback("deletePreference is only supported on iOS platform");
            }
       }
   }
