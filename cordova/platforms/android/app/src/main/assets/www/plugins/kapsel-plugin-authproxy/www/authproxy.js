...
AuthProxy.prototype.generateODataHttpClient2 = function () {
   var httpClient = {
        request: function (request, success, error) {
            var url, requestHeaders, requestBody, statusCode, statusText, responseHeaders;
            var responseBody, requestTimeout
            var client, result;
            ...
            //if caller provides auth config, or logon plugin is not available, then continue without it,
            //otherwise, get default authconfig from logon plugin for authentication
            if (request.authConfig || (!(sap && sap.Logon))) {
                client = AuthProxy.prototype.sendRequest2(request.method || "GET", url, requestHeaders, requestBody, successCB, errorCB, requestTimeout, request.authConfig);
            }
            else{
                var configHandler = function(jsonconfig) {
                    ...
                    client = AuthProxy.prototype.sendRequest2(request.method || "GET", url, requestHeaders, requestBody, successCB, errorCB, requestTimeout, authConfig);
                };
                //if failed to get authconfig, just continue without it
                sap.Logon.getConfiguration(configHandler, function(){configHandler()}, "authentication");
            }
...
/**
 * Sets whether the AuthProxy plugin will attempt to handle X509 challenges for the webview.
 * On Android platform, if this method is never called, the default behaviour is to go by the preference in the config.xml
 * SAPKapselHandleX509Challenges. If that preference is not set, the default is to handle X509
 * challenges.
 * This method is supported only on Android and ios platforms.  If called on any other platform the success callback
 * will be called immediately (to eliminate the need for verbose platform checks elsewhere).
 * @param {Function} successCallback Callback method for when the option is set.
 * @param {Function} errorCallback Callback method for when there is an error.
 * @param {Boolean} handleChallenges True means X509 challenges will be handled.  False means X509
 *                                   challenges will be ignored.
 */
AuthProxy.prototype.setHandleWebviewX509Challenges = function(successCallback, errorCallback, handleChallenges) {
    if (device.platform == "Android"  || device.platform == "iOS") {
        exec(successCallback, errorCallback, "AuthProxy", "setHandleX509", [handleChallenges]);
    } else {
        successCallback();
    }
}
...
//For local html file loaded by ios WKWebView, set authproxy as default oData Http Client.
//As unlike UIWebView, WKWebview does not allow the xmlhttprequest to send cross domain reqeusts to remote server.
if (cordova.require("cordova/platform").id == "ios") {
    if (window.webkit && window.webkit.messageHandlers && (window.location.protocol === "file:")) {
        if ((typeof OData == "undefined") && (typeof jQuery != 'undefined') && jQuery.sap){
            //if Odata module is not yet loaded, try to load it from UI5 library if available
            jQuery.sap.require("sap.ui.thirdparty.datajs");
        }
        if (typeof OData != "undefined"){
           OData.defaultHttpClient = AuthProxyPlugin.generateODataHttpClient2();
        }
    }
}
