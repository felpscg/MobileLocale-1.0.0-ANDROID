...
SettingsExchange.prototype.initWhiteList = function(oncomplete) {
        var done = function() {
	    // oncomplete could be an event if called from addEventListener
            if (oncomplete && typeof oncomplete === "function") {
                oncomplete();
            }
        }
        var theUrl = null;
        var makeSureJson = function(value) {
        ...
        if (window.fiori_client_appConfig && window.fiori_client_appConfig.finalConfig && window.fiori_client_appConfig.finalConfig.noBridgewhitelist) {
            theUrl = window.fiori_client_appConfig.finalConfig.noBridgewhitelist;
            theUrl = makeSureJson(theUrl);
            exec(done, done, "SMPSettingsExchangePlugin", "setWhiteList" , theUrl);
        } else {
                   ...
                   function(whitelist){
                       if ((typeof(whitelist) != 'undefined') && (whitelist != null) && (whitelist.length > 0))
                       {
                           ...
                           if (whitelistArray != null) {
                             theUrl = makeSureJson(whitelistArray);
                             exec(done, done, "SMPSettingsExchangePlugin", "setWhiteList" , theUrl);
                           } else {
                               done();
                           }
