...
- (void)openInInAppBrowser:(NSURL*)url withOptions:(NSString*)options
{
    ...
    if (self.inAppBrowserViewController == nil) {
        ...
        //kapsel change start
        // Look for AuthProxy to configure WKWebView
        CDVViewController* vc = (CDVViewController*)self.viewController;
        for (NSString* pluginName in vc.pluginObjects) {
            CDVPlugin* plugin = [vc.pluginObjects objectForKey:pluginName];
            SEL selector = NSSelectorFromString(@"setupXHRBridge:");
            if ([plugin respondsToSelector:selector]) {
                self.inAppBrowserViewController.xhrBridge = (((NSObject* (*)(id, SEL, id))objc_msgSend)(plugin, selector, self.inAppBrowserViewController.webView));
                SEL selector2 = NSSelectorFromString(@"setupNavigationDelegate:");
                self.webViewDelegate = (id <WKNavigationDelegate>)(((NSObject* (*)(id, SEL, id))objc_msgSend)(plugin, selector2, self.inAppBrowserViewController.webView));
            }
        }
        //kapsel change end
    }
...
- (BOOL)webView:(WKWebView*)theWebView decidePolicyForNavigationAction:(NSURLRequest*)request
{
    ...
    //kapsel change start
    //SAP authenticator can be configured to automatically redirect to/from Fiori client app
    //1970092481, [FC][IOS][WK] Generate code link doesn't work in two factor authentication
    else if ([[url scheme] isEqualToString:@"sapauthenticator"] ){
        [theWebView stopLoading];
        [self openInSystem:url];
        return NO;
    }
...
- (void)createViews
{
    ...
    NSLog(@"Using WKWebView for inappbrowser");
    ...
    */
    if (_browserOptions.hidenavigationbuttons) {
        if(_browserOptions.showprintoption){
            [self.toolbar setItems:@[self.closeButton, flexibleSpaceButton, printButton]];
        }
        else{
            [self.toolbar setItems:@[self.closeButton, flexibleSpaceButton]];
        }
        ...
        if(_browserOptions.showprintoption){
            [self.toolbar setItems:@[self.closeButton, flexibleSpaceButton, printButton, flexibleSpaceButton,  self.backButton, fixedSpaceButton, self.forwardButton]];
        }
        else{
            [self.toolbar setItems:@[self.closeButton, flexibleSpaceButton, self.backButton, fixedSpaceButton, self.forwardButton]];
        }
...
{
//kapsel change start
    //1770482426, [FC][IOS] Using WKWebivew View log page doesn't show any logs
    if ( [url isFileURL]){
...
- (void)webView:(WKWebView *)theWebView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    //kapsel change start
    BOOL bAllow = [self.navigationDelegate webView:theWebView decidePolicyForNavigationAction:navigationAction.request];
