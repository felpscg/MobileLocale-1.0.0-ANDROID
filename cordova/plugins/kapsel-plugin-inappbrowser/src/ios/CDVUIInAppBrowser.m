...
//kapsel change start
@interface CDVUIInAppBrowser () <SFSafariViewControllerDelegate>{
//kapsel change end
...
- (void)browserExit
{
    ...
    */
    ...
    //kapsel change start
    UIWebView* uiWebView = (UIWebView*)self.webViewEngine.engineWebView;
    NSNumber* allowInlineMediaPlayback = [self.commandDelegate.settings valueForKey:@"allowinlinemediaplayback"];
    if (allowInlineMediaPlayback == nil) {
         allowInlineMediaPlayback = [NSNumber numberWithBool:NO];
    }
    uiWebView.allowsInlineMediaPlayback = [allowInlineMediaPlayback boolValue];
    //kapsel change end
}
...
- (void)createViews
{
    ...
    NSLog(@"Using UIWebView for inappbrowser");
        ...
        if ([weakSelf respondsToSelector:@selector(presentingViewController)]) {
            ...
            */
            [[weakSelf presentingViewController] dismissViewControllerAnimated:NO completion:^{
                if (weakSelf.navigationDelegate && weakSelf.navigationDelegate.callbackId) {
                   CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK                                                                  messageAsDictionary:@{@"type":@"exit"}];
                   [weakSelf.navigationDelegate.commandDelegate sendPluginResult:pluginResult callbackId:self.navigationDelegate.callbackId];
